    ...
    <header className="border-b border-gray-800 bg-[#071018]">
      <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="w-12 h-12 rounded-lg flex items-center justify-center bg-panel shadow">
            <span className="text-xl font-bold text-white">SGB</span>
          </div>
          <div>
        ...
        <nav className="flex items-center gap-3">
          ...
          {user && user.role === 'Bibliotecário' && (
            <button onClick={() => onNavigate('painel')} className="hover:text-accent">Painel</button>)}

          {user && user.role === 'Administrador' && (
            <button onClick={() => onNavigate('admin')} className="hover:text-accent">Admin</button>)}
